...
int menu(int x) {
	...
	if (x == 1) {
		table();
		char fname[32], mname[32], lname[32], age[32], gender[32], lrn[32], bmonth[32], bday[32], byear[32], barangay[32], municipality[32], province[32], postalzip[32],
			gfname[32], gmname[32], glname[32], gcnum[32], grelation[32];
		...
		cout << "Birthdate : "; getchcout(bmonth); cout << " "; getchcout(bday); getchcout(byear); cout << " \n";
